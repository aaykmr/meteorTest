...
export const Create = () => {
  ...
  var response = Object.keys(AllPosts.find("collection").collection._docs._map)
    .map((key) => [
      Number(key),
      AllPosts.find("collection").collection._docs._map[key],
    ])
    .sort(GetSortOrder("date"));
  ...
  const submit = (e) => {
    ...
    response = Object.keys(AllPosts.find("collection").collection._docs._map)
      .map((key) => [
        Number(key),
        AllPosts.find("collection").collection._docs._map[key],
      ])
  ...
  return (
    <div className="create">
      ...
      ></textarea>
      <button className="button postBtn" onClick={submit} type="submit">
        Post
      </button>
      ...
      <div className="postBox">
        {result.map((element) => (
          <div className="post">
            ...
            <div className="date">
              {new Date(element[1].date).toDateString()}
            </div>
          </div>
        ))}
      </div>
